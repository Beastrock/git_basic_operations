...
Перед прочтением лучше ознакомиться с ссылками в предыдущей статье на девмане, пройдя туториал гитхаба или прочитав пару глав Git-book'a.  
...
В PyCharm можно выполнять данные операции удобнее и нагляднее чем в консоле. [Ссылка на документацию](https://www.jetbrains.com/help/pycharm/2016.1/using-
-integration.html)   
...
`$ git merge work_branch` # мерджим ветку work_branch  
...
`$ git checkout dev` # переходим в векту dev  
`$ git merge my_branch` # теперь уже сливаем dev с my_branch  
...
Используем [git cherry-pick](https://git-scm.com/docs/git-cherry-pick) и [git reset](https://git-scm.com/book/ru/v2/%D0%98%D0%BD%D1%81%D1%82%D1%80%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D1%8B-Git-%D0%A0%D0%B0%D1%81%D0%BA%D1%80%D1%8B%D1%82%D0%B8%D0%B5-%D1%82%D0%B0%D0%B9%D0%BD-reset):  
`$ git cherry-pick 62ecb3` # выполним команду, находясь в ветке dev. Данная команда заберёт всё текущее состояние ветки в другую одним коммитом, в нашем случае из dev в master. `62ecb3` - хэш ошибочного коммита  
`$ git сommit -m "Commit message`  # сделать коммит уже в своей ветке  
`$ git checkout master` # переходим в master  
`$ git reset --hard HEAD` # удаление ошибочного коммита вместе с индексированными файлами  
...
Первый способ:     
`$ git reset HEAD~2` # произойдет отмена коммитов и все индексированные файлы станут `unstaged`, а состояние репозитория станет "C".   
Исправив свою ошибку в коде, делаем уже привычные команды;
`$ git add index.html`  
`$ git commit -m 'Change header style`  

Также можно использовать `git commit -c ORIG_HEAD`, чтобы не писать то же самое сообщение коммита заново.  

Второй способ:  
`$ git commit revert HEAD~2` # всё тоже самое, но безопаснее: команда создаёт новый коммит c состоянием репозитория "C", не удаляя предыдущие коммиты и не трогая индексные файлы. 

Подробнее о других способах и полезных дополнительных аргументах можно прочесть [здесь](https://githowto.com/ru/undoing_committed_changes).   
...
Еще на [СтекОверфловиче](http://stackoverflow.com/questions/8981194/changing-git-commit-message-after-push-given-that-no-one-pulled-from-remote) объясняется про push с плюсом.  
...
--------------------------------------------------
И на последок для усвоения материала cтатья на ХабраХабре - ["Ежедневный Git"](https://habrahabr.ru/post/28268/).  

# Contributing

Нашел ошибки? Хочешь пулреквестнуть? Пиши мне в slack @beastrock или [вконтакте](http://vk.com/beastrock).
